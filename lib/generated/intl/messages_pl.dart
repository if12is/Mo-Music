// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a pl locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes
// ignore_for_file:unnecessary_string_interpolations, unnecessary_string_escapes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'pl';

  static String m0(songTitle) => "Pobieranie: ${songTitle}";

  static String m1(count) => "Albumy: ${count}";

  static String m2(count) => "Artyści: ${count}";

  static String m3(count) => "Ulubione: ${count}";

  static String m4(count) => "Playlisty: ${count}";

  static String m5(count) => "Piosenki: ${count}";

  static String m6(source) => "Migracja zakończona z ${source}.";

  static String m7(error) => "Wystąpił błąd podczas regeneracji: ${error}";

  static String m8(title) => "Podobny do ${title}";

  static String m9(current) => "Krok _${current} z 3";

  static String m10(count) => "${count} zmiany zatwierdzone.";

  static String m11(count) => "${count} zsynchronizowane zmiany.";

  static String m12(path) => "Backup odzyskiwania: ${path}";

  static String m13(statusCode) =>
      "Nie można wyszukać użytkowników (${statusCode}).";

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
    "CreateNewPlaylist": MessageLookupByLibrary.simpleMessage(
      "Utwórz nową playlistę",
    ),
    "Piped": MessageLookupByLibrary.simpleMessage("Piped"),
    "about": MessageLookupByLibrary.simpleMessage("O"),
    "add5Minutes": MessageLookupByLibrary.simpleMessage("Dodaj 5 minut"),
    "addMultipleSongs": MessageLookupByLibrary.simpleMessage(
      "Dodaj utwory do listy odtwarzania",
    ),
    "addToLibrary": MessageLookupByLibrary.simpleMessage("Dodaj do biblioteki"),
    "addToPlaylist": MessageLookupByLibrary.simpleMessage("Dodaj do playlisty"),
    "album": MessageLookupByLibrary.simpleMessage("Album"),
    "albumBookmarkAddAlert": MessageLookupByLibrary.simpleMessage(
      "Dodano Album do zakładek!",
    ),
    "albumBookmarkRemoveAlert": MessageLookupByLibrary.simpleMessage(
      "Usunięto Album z zakładek!",
    ),
    "albums": MessageLookupByLibrary.simpleMessage("Albumy"),
    "albumsByTaste": MessageLookupByLibrary.simpleMessage(
      "Według Twoich upodobań",
    ),
    "allFieldsReqMsg": MessageLookupByLibrary.simpleMessage(
      "Wymagane są wszystkie pola",
    ),
    "androidBackupWarning": MessageLookupByLibrary.simpleMessage(
      "Nie przetestowano: Zaznaczenie pola wyboru po pobraniu ponad 60 plików może spowodować zużycie dużej ilości pamięci i awarię telefonu lub aplikacji. Postępujesz na własne ryzyko.",
    ),
    "appInfo": MessageLookupByLibrary.simpleMessage("Informacje o aplikacji"),
    "artistBookmarkAddAlert": MessageLookupByLibrary.simpleMessage(
      "Dodano Wykonawcę do zakładek!",
    ),
    "artistBookmarkRemoveAlert": MessageLookupByLibrary.simpleMessage(
      "Usunięto Wykonawcę do zakładek!",
    ),
    "artistDesNotAvailable": MessageLookupByLibrary.simpleMessage(
      "Opis jest niedostępny!",
    ),
    "artists": MessageLookupByLibrary.simpleMessage("Wykonawcy"),
    "artistsByTaste": MessageLookupByLibrary.simpleMessage(
      "Według Twoich upodobań",
    ),
    "audioCodec": MessageLookupByLibrary.simpleMessage("Kodek dźwięku"),
    "auth_2fa_code": MessageLookupByLibrary.simpleMessage(
      "Kod uwierzytelniający",
    ),
    "auth_2fa_invalid": MessageLookupByLibrary.simpleMessage(
      "Wprowadź ważny 6-cyfrowy kod lub zaloguj się ponownie.",
    ),
    "auth_2fa_subtitle": MessageLookupByLibrary.simpleMessage(
      "Wprowadź 6-cyfrowy kod z aplikacji uwierzytelniającej. Dostęp ten wygasa za 5 minut.",
    ),
    "auth_2fa_title": MessageLookupByLibrary.simpleMessage(
      "Uwierzytelnianie dwuskładnikowe",
    ),
    "auth_2fa_verify": MessageLookupByLibrary.simpleMessage(
      "Sprawdź i kontynuuj",
    ),
    "auth_agree_personal_data": MessageLookupByLibrary.simpleMessage(
      "Akceptuj moje dane...",
    ),
    "auth_brand_description_1": MessageLookupByLibrary.simpleMessage(
      "Przynieśliśmy logowanie, rejestrację i odzyskiwanie hasła z poprzedniego projektu, dostosowane do tej aplikacji muzycznej.",
    ),
    "auth_brand_description_2": MessageLookupByLibrary.simpleMessage(
      "Twoja sesja jest przechowywana w bezpiecznym magazynie i jest sprawdzana przy użyciu tego samego backendu, którego już używałeś.",
    ),
    "auth_brand_not_configured": MessageLookupByLibrary.simpleMessage(
      "Aby połączyć moduł uwierzytelniania, należy skonfigurować plik .env.",
    ),
    "auth_btn_login": MessageLookupByLibrary.simpleMessage("Login"),
    "auth_btn_register": MessageLookupByLibrary.simpleMessage("Rejestr"),
    "auth_btn_send_email": MessageLookupByLibrary.simpleMessage(
      "Wyślij pocztę",
    ),
    "auth_confirm_password": MessageLookupByLibrary.simpleMessage(
      "Potwierdź hasło",
    ),
    "auth_error_invalid_credentials": MessageLookupByLibrary.simpleMessage(
      "Nieprawidłowy adres e-mail lub hasło.",
    ),
    "auth_error_invalid_email": MessageLookupByLibrary.simpleMessage(
      "Wpisz prawidłowy adres e-mail.",
    ),
    "auth_error_not_configured": MessageLookupByLibrary.simpleMessage(
      "Brakuje modułu uwierzytelniania do skonfigurowania w pliku .env.",
    ),
    "auth_error_not_verified": MessageLookupByLibrary.simpleMessage(
      "Twoje konto nie zostało jeszcze zweryfikowane.",
    ),
    "auth_error_unknown": MessageLookupByLibrary.simpleMessage(
      "Nie udało się dokończyć operacji.",
    ),
    "auth_first_name": MessageLookupByLibrary.simpleMessage("Imię"),
    "auth_forgot_password": MessageLookupByLibrary.simpleMessage(
      "Zapomniałem hasła",
    ),
    "auth_forgot_password_subtitle": MessageLookupByLibrary.simpleMessage(
      "Wyślemy Ci instrukcje na adres e-mail Twojego konta.",
    ),
    "auth_hint_email": MessageLookupByLibrary.simpleMessage("nazwa@mail.com"),
    "auth_last_name": MessageLookupByLibrary.simpleMessage("Nazwisko"),
    "auth_login_success": MessageLookupByLibrary.simpleMessage(
      "Zalogowano się pomyślnie",
    ),
    "auth_recovery_email_error": MessageLookupByLibrary.simpleMessage(
      "Nie można było wysłać e-maila.",
    ),
    "auth_recovery_email_sent": MessageLookupByLibrary.simpleMessage(
      "E-mail wysłany.",
    ),
    "auth_register_error": MessageLookupByLibrary.simpleMessage(
      "Nie udało się utworzyć konta.",
    ),
    "auth_register_success": MessageLookupByLibrary.simpleMessage(
      "Konto utworzone pomyślnie.",
    ),
    "auth_welcome_subtitle": MessageLookupByLibrary.simpleMessage(
      "Witamy w Mo Music",
    ),
    "auth_welcome_title": MessageLookupByLibrary.simpleMessage(
      "Witamy w Mo Music",
    ),
    "autoDownFavSong": MessageLookupByLibrary.simpleMessage(
      "Automatyczne pobieranie ulubionych utworów",
    ),
    "autoDownFavSongDes": MessageLookupByLibrary.simpleMessage(
      "Automatyczne pobieranie ulubionych utworów po dodaniu ich do ulubionych",
    ),
    "autoOpenPlayer": MessageLookupByLibrary.simpleMessage(
      "Automatyczne otwieranie ekranu odtwarzacza",
    ),
    "autoOpenPlayerDes": MessageLookupByLibrary.simpleMessage(
      "Włączenie/wyłączenie automatycznego otwierania odtwarzacza na pełnym ekranie po wybraniu utworu do odtwarzania",
    ),
    "back": MessageLookupByLibrary.simpleMessage("Powrót"),
    "backFilesFound": MessageLookupByLibrary.simpleMessage(
      "znalezione bazy danych",
    ),
    "backgroundPlay": MessageLookupByLibrary.simpleMessage(
      "Odtwarzanie muzyki w tle",
    ),
    "backgroundPlayDes": MessageLookupByLibrary.simpleMessage(
      "Włączanie/wyłączanie odtwarzania muzyki w tle (aplikacja jest dostępna z zasobnika systemowego, gdy aplikacja działa w tle)",
    ),
    "backup": MessageLookupByLibrary.simpleMessage("Kopia zapasowa"),
    "backupAppData": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa danych aplikacji",
    ),
    "backupInProgress": MessageLookupByLibrary.simpleMessage(
      "Trwa tworzenie kopii zapasowej...",
    ),
    "backupMsg": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa została pomyślnie zapisana!",
    ),
    "backupSettingsAndPlaylists": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa ustawień i list odtwarzania",
    ),
    "backupSettingsAndPlaylistsDes": MessageLookupByLibrary.simpleMessage(
      "Zapisuje wszystkie ustawienia, listy odtwarzania i dane logowania w pliku kopii zapasowej",
    ),
    "backup_auth_required": MessageLookupByLibrary.simpleMessage(
      "Potrzebujesz aktywnej sesji...",
    ),
    "backup_btn_restart": MessageLookupByLibrary.simpleMessage(
      "Uruchom ponownie aplikację",
    ),
    "backup_btn_upload": MessageLookupByLibrary.simpleMessage(
      "Prześlij teraz kopię zapasową",
    ),
    "backup_confirm_question": MessageLookupByLibrary.simpleMessage(
      "Czy chcesz wykonać kopię zapasową?",
    ),
    "backup_delete_success": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa została usunięta.",
    ),
    "backup_no_backups": MessageLookupByLibrary.simpleMessage(
      "Nie ma jeszcze żadnych kopii zapasowych...",
    ),
    "backup_restore_success": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa przywrócona. Uruchom ponownie aplikację.",
    ),
    "backup_select_folder_dialog": MessageLookupByLibrary.simpleMessage(
      "Wybierz folder do utworzenia kopii zapasowej",
    ),
    "backup_selection_prompt": MessageLookupByLibrary.simpleMessage(
      "Wybierz, które dane chcesz wykonać w kopii zapasowej",
    ),
    "backup_upload_success": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa przesłana poprawnie.",
    ),
    "basedOnLast": MessageLookupByLibrary.simpleMessage(
      "Na podstawie ostatniej interakcji",
    ),
    "bitrate": MessageLookupByLibrary.simpleMessage("Szybkość transmisji"),
    "blacklistPipedPlaylist": MessageLookupByLibrary.simpleMessage(
      "Czarna lista playlist",
    ),
    "blacklistPlstResetAlert": MessageLookupByLibrary.simpleMessage(
      "Zresetowano pomyślnie!",
    ),
    "by": MessageLookupByLibrary.simpleMessage("autorstwa"),
    "cacheHomeScreenData": MessageLookupByLibrary.simpleMessage(
      "Pamięć podręczna danych zawartości ekranu głównego",
    ),
    "cacheHomeScreenDataDes": MessageLookupByLibrary.simpleMessage(
      "Włącz buforowanie danych zawartości ekranu głównego, ekran główny załaduje się natychmiast, jeśli ta opcja jest włączona",
    ),
    "cacheSongs": MessageLookupByLibrary.simpleMessage(
      "Utwory w pamięci podręcznej",
    ),
    "cacheSongsDes": MessageLookupByLibrary.simpleMessage(
      "Pozwól na pobieranie otwieranych utworów do pamięci podręcznej podczas odtwarzania, aby odtworzyć je w przyszłości. Pliki offline zajmą dodatkowe miejsce w pamięci urządzenia",
    ),
    "cachedOrOffline": MessageLookupByLibrary.simpleMessage(
      "Pamięć podręczna/Oflline",
    ),
    "cancel": MessageLookupByLibrary.simpleMessage("Anuluj"),
    "cancelTimer": MessageLookupByLibrary.simpleMessage(
      "Anuluj wyłącznik czasowy",
    ),
    "cancelTimerAlert": MessageLookupByLibrary.simpleMessage(
      "Wyłącznik uśpienia anulowany",
    ),
    "clearImgCache": MessageLookupByLibrary.simpleMessage(
      "Wyczyść pamięć podręczną obrazów",
    ),
    "clearImgCacheAlert": MessageLookupByLibrary.simpleMessage(
      "Pomyślnie wyczyszczono pamięć podręczną obrazów",
    ),
    "clearImgCacheDes": MessageLookupByLibrary.simpleMessage(
      "Kliknij tutaj, aby wyczyścić zapisane w pamięci podręcznej miniatury/obrazy. (Niezalecane, chyba że chcesz odświeżyć dane obrazów zapisanych w pamięci podręcznej)",
    ),
    "close": MessageLookupByLibrary.simpleMessage("Zamknąć"),
    "closeApp": MessageLookupByLibrary.simpleMessage("Zamknij aplikację"),
    "cloudLibraryFound": MessageLookupByLibrary.simpleMessage(
      "Znaleziono bibliotekę chmurową.",
    ),
    "cloudLibraryFoundDeviceWillDownload": MessageLookupByLibrary.simpleMessage(
      "Znaleziono bibliotekę w chmurze. To urządzenie pobierze je bez nadpisywania.",
    ),
    "cloudModeReadyOfflineCache": MessageLookupByLibrary.simpleMessage(
      "Tryb chmury jest gotowy. To urządzenie będzie działać jako pamięć podręczna offline.",
    ),
    "cloud_b1": MessageLookupByLibrary.simpleMessage(
      "Zaloguj się bezpiecznie, korzystając ze swojego konta Joss Red.",
    ),
    "cloud_b2": MessageLookupByLibrary.simpleMessage(
      "Uzyskaj natychmiastowy dostęp do swoich list odtwarzania, ulubionych i historii z dowolnego urządzenia (Windows, Android itp.).",
    ),
    "cloud_b3": MessageLookupByLibrary.simpleMessage(
      "Inteligentna synchronizacja: Pracuj w trybie offline i automatycznie przesyłaj zmiany po odzyskaniu Internetu.",
    ),
    "cloud_btn": MessageLookupByLibrary.simpleMessage(
      "Aktywuj synchronizację z chmurą",
    ),
    "cloud_subtitle": MessageLookupByLibrary.simpleMessage(
      "Synchronizacja w czasie rzeczywistym z Joss Red",
    ),
    "cloud_title": MessageLookupByLibrary.simpleMessage(
      "Tryb chmury (zalecany)",
    ),
    "collaborativePlaylistDescription": MessageLookupByLibrary.simpleMessage(
      "Wspólna playlista",
    ),
    "collaboratorsInstruction": MessageLookupByLibrary.simpleMessage(
      "Wybierz znajomych, którzy będą mogli oglądać i edytować tę playlistę:",
    ),
    "collaboratorsUpdated": MessageLookupByLibrary.simpleMessage(
      "Współpracownicy zaktualizowani poprawnie.",
    ),
    "communityplaylists": MessageLookupByLibrary.simpleMessage(
      "Społecznościowe playlisty",
    ),
    "content": MessageLookupByLibrary.simpleMessage("Zawartość"),
    "copyrightNotice": MessageLookupByLibrary.simpleMessage(
      "© 2026 JOSPROX. Licencja GPL v3.0",
    ),
    "create": MessageLookupByLibrary.simpleMessage("Utwórz"),
    "createnAdd": MessageLookupByLibrary.simpleMessage("Utwórz i dodaj"),
    "customIns": MessageLookupByLibrary.simpleMessage(
      "Niestandardowe wystąpienie",
    ),
    "customInsSelectMsg": MessageLookupByLibrary.simpleMessage(
      "Proszę wybierz niestandardową Instancję",
    ),
    "dailyDiscover": MessageLookupByLibrary.simpleMessage("Codzienne odkrycie"),
    "dark": MessageLookupByLibrary.simpleMessage("Ciemny"),
    "delete": MessageLookupByLibrary.simpleMessage("Usuń"),
    "deleteDownloadData": MessageLookupByLibrary.simpleMessage(
      "Usuń z pobranych",
    ),
    "deleteDownloadedDataAlert": MessageLookupByLibrary.simpleMessage(
      "Pomyślnie usunięto z pobranych!",
    ),
    "developedBy": MessageLookupByLibrary.simpleMessage(
      "Opracowany i utrzymywany przez Jossa Estradę (JOSPROX)",
    ),
    "disableTransitionAnimation": MessageLookupByLibrary.simpleMessage(
      "Wyłącz animację przejścia",
    ),
    "disableTransitionAnimationDes": MessageLookupByLibrary.simpleMessage(
      "Włącz tę opcję, aby wyłączyć animację przejścia między kartami",
    ),
    "disabled": MessageLookupByLibrary.simpleMessage("Wyłączony"),
    "discover": MessageLookupByLibrary.simpleMessage("Odkrywaj"),
    "dismiss": MessageLookupByLibrary.simpleMessage("Odrzuć"),
    "done": MessageLookupByLibrary.simpleMessage("Gotowy"),
    "dontShowInfoAgain": MessageLookupByLibrary.simpleMessage(
      "Nie pokazuj więcej tej informacji",
    ),
    "downFilesFound": MessageLookupByLibrary.simpleMessage(
      "znaleziono pobrane pliki",
    ),
    "download": MessageLookupByLibrary.simpleMessage("Pobierz"),
    "downloadAlbumSongs": MessageLookupByLibrary.simpleMessage(
      "Pobierz utwory z albumu",
    ),
    "downloadError2": MessageLookupByLibrary.simpleMessage(
      "Żądany utwór nie może zostać pobrany z powodu ograniczeń serwera. Możesz spróbować ponownie",
    ),
    "downloadError3": MessageLookupByLibrary.simpleMessage(
      "Pobieranie nie powiodło się z powodu błędu sieci/strumienia! Spróbuj ponownie",
    ),
    "downloadLocation": MessageLookupByLibrary.simpleMessage(
      "Lokalizacja Pobierania",
    ),
    "downloadNotificationChannelDescription":
        MessageLookupByLibrary.simpleMessage(
          "Utrzymuje pobieranie muzyki aktywne w tle.",
        ),
    "downloadNotificationChannelName": MessageLookupByLibrary.simpleMessage(
      "pobieranie muzyki",
    ),
    "downloadNotificationPreparing": MessageLookupByLibrary.simpleMessage(
      "Przygotowuję pliki do pobrania…",
    ),
    "downloadNotificationSong": m0,
    "downloadNotificationTitle": MessageLookupByLibrary.simpleMessage(
      "Pobieranie muzyki",
    ),
    "downloadPlaylist": MessageLookupByLibrary.simpleMessage(
      "Pobierz playlistę",
    ),
    "downloadingFormat": MessageLookupByLibrary.simpleMessage(
      "Pobierz format pliku",
    ),
    "downloadingFormatDes": MessageLookupByLibrary.simpleMessage(
      "Wybierz format pliku do pobrania. „Opus” zapewni najlepszą jakość",
    ),
    "downloads": MessageLookupByLibrary.simpleMessage("Pobrania"),
    "duration": MessageLookupByLibrary.simpleMessage("Czas trwania"),
    "dynamic": MessageLookupByLibrary.simpleMessage("Dynamiczny"),
    "email": MessageLookupByLibrary.simpleMessage("E-mail"),
    "emptyPlaylist": MessageLookupByLibrary.simpleMessage(
      "Playlista jest pusta!",
    ),
    "enableBottomNav": MessageLookupByLibrary.simpleMessage(
      "Dolny pasek nawigacyjny",
    ),
    "enableBottomNavDes": MessageLookupByLibrary.simpleMessage(
      "Przełącz na dolny pasek nawigacyjny",
    ),
    "enableSlidableAction": MessageLookupByLibrary.simpleMessage(
      "Włącz przesuwalne akcje",
    ),
    "enableSlidableActionDes": MessageLookupByLibrary.simpleMessage(
      "Włącz przesuwalne akcje na kafelku utworu",
    ),
    "enabled": MessageLookupByLibrary.simpleMessage("Włączony"),
    "endOfThisSong": MessageLookupByLibrary.simpleMessage(
      "Koniec tej piosenki",
    ),
    "enqueueAlbumSongs": MessageLookupByLibrary.simpleMessage(
      "Dodaj utwory z albumu do kolejki",
    ),
    "enqueueAll": MessageLookupByLibrary.simpleMessage("Kolejkuj wszystkie"),
    "enqueueSong": MessageLookupByLibrary.simpleMessage(
      "Dodaj ten utwór do kolejki",
    ),
    "enqueueSongs": MessageLookupByLibrary.simpleMessage(
      "Dodaj utwory do kolejki",
    ),
    "episodes": MessageLookupByLibrary.simpleMessage("Odcinki"),
    "equalizer": MessageLookupByLibrary.simpleMessage("Korektor dźwięku"),
    "equalizerDes": MessageLookupByLibrary.simpleMessage(
      "Otwórz korektor systemowy",
    ),
    "errorOccuredAlert": MessageLookupByLibrary.simpleMessage("Wystąpił błąd!"),
    "errorOccurred": MessageLookupByLibrary.simpleMessage("Wystąpił błąd"),
    "errorPlayingTrack": MessageLookupByLibrary.simpleMessage(
      "Błąd podczas odtwarzania:",
    ),
    "export": MessageLookupByLibrary.simpleMessage("Eksportuj"),
    "exportDowloadedFiles": MessageLookupByLibrary.simpleMessage(
      "Eksportuj pobrane pliki",
    ),
    "exportDowloadedFilesDes": MessageLookupByLibrary.simpleMessage(
      "Kliknij tutaj, aby wyeksportować pobrany plik z katalogu inApp do zewnętrznego katalogu",
    ),
    "exportError": MessageLookupByLibrary.simpleMessage(
      "Błąd podczas eksportowania listy odtwarzania",
    ),
    "exportErrorFormat": MessageLookupByLibrary.simpleMessage(
      "Błąd formatowania danych listy odtwarzania",
    ),
    "exportErrorPermission": MessageLookupByLibrary.simpleMessage(
      "Odmowa dostępu podczas eksportowania",
    ),
    "exportErrorStorage": MessageLookupByLibrary.simpleMessage(
      "Za mało wolnego miejsca na dysku",
    ),
    "exportMsg": MessageLookupByLibrary.simpleMessage(
      "Pliki zostały pomyślnie wyeksportowane",
    ),
    "exportPlaylist": MessageLookupByLibrary.simpleMessage(
      "Eksportuj listę odtwarzania",
    ),
    "exportPlaylistCsv": MessageLookupByLibrary.simpleMessage(
      "Eksportuj playlistę jako CSV",
    ),
    "exportPlaylistCsvSubtitle": MessageLookupByLibrary.simpleMessage(
      "Nie można importować tutaj",
    ),
    "exportPlaylistJson": MessageLookupByLibrary.simpleMessage(
      "Eksportuj playlistę do JSON",
    ),
    "exportPlaylistJsonSubtitle": MessageLookupByLibrary.simpleMessage(
      "Ten format można importować",
    ),
    "exportToOnlineMusic": MessageLookupByLibrary.simpleMessage(
      "Eksportuj do muzyki z Online",
    ),
    "exportToOnlineMusicSubtitle": MessageLookupByLibrary.simpleMessage(
      "Spowoduje to przesunięcie Twojej playlisty (utwory < 50) do bieżącej kolejki. Nie zapomnij dodać jej do playlisty/zapisać po otwarciu w MusicService",
    ),
    "exportedFileLocation": MessageLookupByLibrary.simpleMessage(
      "Lokalizacja eksportu pobranego pliku",
    ),
    "exporting": MessageLookupByLibrary.simpleMessage("Eksportowanie..."),
    "exportingPlaylist": MessageLookupByLibrary.simpleMessage(
      "Eksportowanie listy odtwarzania…",
    ),
    "favorites": MessageLookupByLibrary.simpleMessage("Ulubione"),
    "featuredplaylists": MessageLookupByLibrary.simpleMessage(
      "Polecane playlisty",
    ),
    "fileNotFound": MessageLookupByLibrary.simpleMessage(
      "Plik nie został znaleziony",
    ),
    "follow": MessageLookupByLibrary.simpleMessage("Kontynuować"),
    "followedArtists": MessageLookupByLibrary.simpleMessage("podążał"),
    "following": MessageLookupByLibrary.simpleMessage("Następny"),
    "for1": MessageLookupByLibrary.simpleMessage("dla"),
    "forgottenFavorites": MessageLookupByLibrary.simpleMessage(
      "zapomniane ulubione",
    ),
    "friendFallback": MessageLookupByLibrary.simpleMessage("Przyjaciel"),
    "friendRequestAccepted": MessageLookupByLibrary.simpleMessage(
      "Zaproszenie do grona znajomych zaakceptowane",
    ),
    "friendRequestSent": MessageLookupByLibrary.simpleMessage(
      "Wysłano zaproszenie do grona znajomych",
    ),
    "friends": MessageLookupByLibrary.simpleMessage("Przyjaciele"),
    "friendsLoginRequired": MessageLookupByLibrary.simpleMessage(
      "Zaloguj się, aby znaleźć przyjaciół.",
    ),
    "friendshipRemoved": MessageLookupByLibrary.simpleMessage(
      "Przyjaźń usunięta",
    ),
    "genericAlbum": MessageLookupByLibrary.simpleMessage("Album"),
    "genericError": MessageLookupByLibrary.simpleMessage("Błąd"),
    "genre_electronic": MessageLookupByLibrary.simpleMessage("Elektronika"),
    "genre_hiphop": MessageLookupByLibrary.simpleMessage("hip-hop"),
    "genre_jazz": MessageLookupByLibrary.simpleMessage("Jazz"),
    "genre_latin": MessageLookupByLibrary.simpleMessage("łacina"),
    "genre_pop": MessageLookupByLibrary.simpleMessage("Muzyka pop"),
    "genre_rock": MessageLookupByLibrary.simpleMessage("Głaz"),
    "gesture": MessageLookupByLibrary.simpleMessage("Gesty"),
    "github": MessageLookupByLibrary.simpleMessage("Github"),
    "githubDes": MessageLookupByLibrary.simpleMessage(
      "Zobacz kod źródłowy na Github, \njeśli podoba Ci się ten projekt, nie zapomnij dać ⭐",
    ),
    "goToAlbum": MessageLookupByLibrary.simpleMessage("Przejdź do albumu"),
    "goToDownloadPage": MessageLookupByLibrary.simpleMessage(
      "Kliknij tutaj, aby przejść do strony pobierania",
    ),
    "helloWorld": MessageLookupByLibrary.simpleMessage("Witaj świecie"),
    "high": MessageLookupByLibrary.simpleMessage("Wysoka"),
    "hintApiUrl": MessageLookupByLibrary.simpleMessage(
      "API URL do instancji Piped",
    ),
    "home": MessageLookupByLibrary.simpleMessage("Główna"),
    "homeContentCount": MessageLookupByLibrary.simpleMessage(
      "Ilość treści na ekranie głównym",
    ),
    "homeContentCountDes": MessageLookupByLibrary.simpleMessage(
      "Wybierz liczbę treści na ekranie głównym (w przybliżeniu). Mniej zawartości = szybsze ładowanie",
    ),
    "id": MessageLookupByLibrary.simpleMessage("Id"),
    "identifySongMetadata": MessageLookupByLibrary.simpleMessage(
      "Identyfikacja metadanych",
    ),
    "ignoreBatOpt": MessageLookupByLibrary.simpleMessage(
      "Zignoruj optymalizację baterii",
    ),
    "ignoreBatOptDes": MessageLookupByLibrary.simpleMessage(
      "Jeśli masz problemy z powiadomieniami lub odtwarzanie zostało zatrzymane przez optymalizację systemu, włącz tę opcję",
    ),
    "importError": MessageLookupByLibrary.simpleMessage(
      "Błąd podczas importowania listy odtwarzania",
    ),
    "importErrorDatabase": MessageLookupByLibrary.simpleMessage(
      "Błąd podczas zapisywania do bazy danych",
    ),
    "importErrorFileAccess": MessageLookupByLibrary.simpleMessage(
      "Nie udało się uzyskać dostępu do wybranego pliku",
    ),
    "importErrorFormat": MessageLookupByLibrary.simpleMessage(
      "Nieprawidłowy format pliku",
    ),
    "importLargeFileNote": MessageLookupByLibrary.simpleMessage(
      "Uwaga: import dużych list odtwarzania może potrwać dłużej",
    ),
    "importPlaylist": MessageLookupByLibrary.simpleMessage(
      "Importuj listę odtwarzania",
    ),
    "importPlaylistDesc": MessageLookupByLibrary.simpleMessage(
      "Wybierz wcześniej wyeksportowany plik playlisty w formacie JSON do zaimportowania",
    ),
    "imported": MessageLookupByLibrary.simpleMessage("Importowane"),
    "importedFromJossMusic": MessageLookupByLibrary.simpleMessage(
      "Zaimportowane z Joss Music Kotlin",
    ),
    "importedPlaylist": MessageLookupByLibrary.simpleMessage(
      "Zaimportowana playlista",
    ),
    "importingPlaylist": MessageLookupByLibrary.simpleMessage(
      "Importowanie listy odtwarzania…",
    ),
    "in_app_storage": MessageLookupByLibrary.simpleMessage(
      "Wewnętrzny katalog pamięci",
    ),
    "includeDownloadedFiles": MessageLookupByLibrary.simpleMessage(
      "Dołącz pliki z pobranymi utworami",
    ),
    "infoNotAvailable": MessageLookupByLibrary.simpleMessage(
      "Informacje niedostępne",
    ),
    "invalidPlaylistFile": MessageLookupByLibrary.simpleMessage(
      "Nieprawidłowa struktura pliku listy odtwarzania",
    ),
    "invalidServerResponse": MessageLookupByLibrary.simpleMessage(
      "Nieprawidłowa odpowiedź serwera.",
    ),
    "invalidSessionToken": MessageLookupByLibrary.simpleMessage(
      "Sesja nie zawiera prawidłowego tokena.",
    ),
    "items": MessageLookupByLibrary.simpleMessage("pozycje"),
    "keepListening": MessageLookupByLibrary.simpleMessage("słuchaj dalej"),
    "keepScreenOnWhilePlaying": MessageLookupByLibrary.simpleMessage(
      "Utrzymuj ekran włączony podczas odtwarzania",
    ),
    "keepScreenOnWhilePlayingDes": MessageLookupByLibrary.simpleMessage(
      "Jeśli włączone, ekran urządzenia pozostanie włączony podczas odtwarzania muzyki",
    ),
    "language": MessageLookupByLibrary.simpleMessage("Język"),
    "languageDes": MessageLookupByLibrary.simpleMessage(
      "Ustaw język aplikacji",
    ),
    "latestRelease": MessageLookupByLibrary.simpleMessage("Najnowsze wydanie"),
    "latestVersion": MessageLookupByLibrary.simpleMessage(
      "Dostępna najnowsza wersja",
    ),
    "letsStrart": MessageLookupByLibrary.simpleMessage("Zaczynajmy..."),
    "libAlbums": MessageLookupByLibrary.simpleMessage("Biblioteka Albumów"),
    "libArtists": MessageLookupByLibrary.simpleMessage("Biblioteka Wykonawców"),
    "libPlaylists": MessageLookupByLibrary.simpleMessage("Biblioteka Playlist"),
    "libSongs": MessageLookupByLibrary.simpleMessage("Biblioteka utworów"),
    "library": MessageLookupByLibrary.simpleMessage("Biblioteka"),
    "libraryPlaylistDescription": MessageLookupByLibrary.simpleMessage(
      "Playlista biblioteczna",
    ),
    "light": MessageLookupByLibrary.simpleMessage("Jasny"),
    "link": MessageLookupByLibrary.simpleMessage("Odnośnik"),
    "linkAlert": MessageLookupByLibrary.simpleMessage("Połączono pomyślnie!"),
    "linkCopied": MessageLookupByLibrary.simpleMessage(
      "Link skopiowany do schowka",
    ),
    "linkPipedDes": MessageLookupByLibrary.simpleMessage(
      "Link z potokiem dla list odtwarzania",
    ),
    "listenNow": MessageLookupByLibrary.simpleMessage("Posłuchaj teraz"),
    "listeningToEnvironment": MessageLookupByLibrary.simpleMessage(
      "Słuchając otoczenia...",
    ),
    "loadInfoUpdate": MessageLookupByLibrary.simpleMessage(
      "Nie można załadować informacji o aktualizacji",
    ),
    "local": MessageLookupByLibrary.simpleMessage("Lokalny"),
    "local_b1": MessageLookupByLibrary.simpleMessage(
      "Działa bez konieczności logowania.",
    ),
    "local_b2": MessageLookupByLibrary.simpleMessage(
      "Cała Twoja biblioteka pozostaje wyłącznie na tym komputerze.",
    ),
    "local_b3": MessageLookupByLibrary.simpleMessage(
      "Uwaga: brak ręcznych kopii zapasowych w chmurze. Jeśli zgubisz urządzenie lub odinstalujesz aplikację, odzyskanie danych będzie niemożliwe.",
    ),
    "local_btn": MessageLookupByLibrary.simpleMessage(
      "Używaj tylko na tym urządzeniu",
    ),
    "local_subtitle": MessageLookupByLibrary.simpleMessage(
      "Całkowita prywatność na Twoim urządzeniu",
    ),
    "local_title": MessageLookupByLibrary.simpleMessage("Tryb lokalny"),
    "loudnessDb": MessageLookupByLibrary.simpleMessage("GłośnośćDb"),
    "loudnessNormalization": MessageLookupByLibrary.simpleMessage(
      "Normalizacja głośności",
    ),
    "loudnessNormalizationDes": MessageLookupByLibrary.simpleMessage(
      "Ustawia ten sam poziom głośności dla wszystkich utworów (eksperymentalne) (nie działa z utworami pobranymi w poprzedniej wersji (< v1.10.0))",
    ),
    "low": MessageLookupByLibrary.simpleMessage("Niska"),
    "lyrics": MessageLookupByLibrary.simpleMessage("Listy"),
    "lyricsNotAvailable": MessageLookupByLibrary.simpleMessage(
      "Tekst jest niedostępny!",
    ),
    "manageCollaborators": MessageLookupByLibrary.simpleMessage(
      "Zarządzaj współpracownikami (znajomymi)",
    ),
    "metadataApplySuccess": MessageLookupByLibrary.simpleMessage(
      "Metadane były osadzone w lokalnym pliku.",
    ),
    "metadataNoResults": MessageLookupByLibrary.simpleMessage(
      "Brak dopasowań. Spróbuj innego wyszukiwania.",
    ),
    "metadataOperationFailed": MessageLookupByLibrary.simpleMessage(
      "Operacja metadanych nie powiodła się.",
    ),
    "metadataOverwriteWarning": MessageLookupByLibrary.simpleMessage(
      "To nadpisze wbudowany tytuł, artysta, album i okładkę, zachowując pola, których mecz nie zapewnia.",
    ),
    "metadataSearchDescription": MessageLookupByLibrary.simpleMessage(
      "Wybierz odpowiedni mecz, aby umieścić jego tytuł, artysta, album i okładkę w pliku lokalnym.",
    ),
    "metadataSearchHint": MessageLookupByLibrary.simpleMessage(
      "Nazwa piosenki lub artysty",
    ),
    "metadataSearchTitle": MessageLookupByLibrary.simpleMessage(
      "Identyfikacja piosenki",
    ),
    "micInstruction": MessageLookupByLibrary.simpleMessage(
      "Upewnij się, że muzyka gra wystarczająco głośno w pobliżu mikrofonu.",
    ),
    "migratedAlbum": MessageLookupByLibrary.simpleMessage("Przeniesiony album"),
    "migratedLibrary": MessageLookupByLibrary.simpleMessage(
      "Przeniesiona biblioteka",
    ),
    "migratedPlaylist": MessageLookupByLibrary.simpleMessage(
      "Przeniesiona playlista",
    ),
    "migrationAlreadyRunning": MessageLookupByLibrary.simpleMessage(
      "Migracja już trwa.",
    ),
    "migrationAnalyzingLocal": MessageLookupByLibrary.simpleMessage(
      "Analizując lokalną bibliotekę...",
    ),
    "migrationCheckingCloud": MessageLookupByLibrary.simpleMessage(
      "Sprawdzanie, czy EMusic Cloud ma już bibliotekę...",
    ),
    "migrationCompleted": MessageLookupByLibrary.simpleMessage(
      "Migracja zakończona.",
    ),
    "migrationCreatingBackup": MessageLookupByLibrary.simpleMessage(
      "Tworzenie lokalnej kopii zapasowej przed połączeniem z chmurą...",
    ),
    "migrationFailedLocalPreserved": MessageLookupByLibrary.simpleMessage(
      "Migracja nie powiodła się. Twoje dane lokalne nie zostały zmodyfikowane.",
    ),
    "migrationLoginRequired": MessageLookupByLibrary.simpleMessage(
      "Przed migracją zaloguj się do Joss Red.",
    ),
    "migrationPreparingCloud": MessageLookupByLibrary.simpleMessage(
      "Przygotowanie migracji w EMusic Cloud...",
    ),
    "migrationStartFailed": MessageLookupByLibrary.simpleMessage(
      "EMusic Cloud nie mógł rozpocząć migracji.",
    ),
    "migrationUploadIncomplete": MessageLookupByLibrary.simpleMessage(
      "Nie wszystkie dane mogły zostać przesłane. Utrzymujemy Twoje lokalne wsparcie.",
    ),
    "migrationUploadingData": MessageLookupByLibrary.simpleMessage(
      "Przesyłanie list odtwarzania, ulubionych i historii...",
    ),
    "migrationValidationFailed": MessageLookupByLibrary.simpleMessage(
      "EMusic Cloud nie mógł zweryfikować migracji.",
    ),
    "migrationVerifyingIntegrity": MessageLookupByLibrary.simpleMessage(
      "Sprawdzanie integralności w chmurze EMusic...",
    ),
    "migration_btn_select": MessageLookupByLibrary.simpleMessage(
      "Wybierz plik i zaimportuj",
    ),
    "migration_select_file_dialog": MessageLookupByLibrary.simpleMessage(
      "Wybierz plik song.db lub plik kopii zapasowej .backup",
    ),
    "migration_success": MessageLookupByLibrary.simpleMessage(
      "Migracja zakończyła się pomyślnie.",
    ),
    "migration_summary_albums": m1,
    "migration_summary_artists": m2,
    "migration_summary_favorites": m3,
    "migration_summary_playlists": m4,
    "migration_summary_songs": m5,
    "migration_summary_start": m6,
    "minutes": MessageLookupByLibrary.simpleMessage("minut"),
    "misc": MessageLookupByLibrary.simpleMessage("Różne"),
    "mostListenedSong": MessageLookupByLibrary.simpleMessage(
      "Najczęściej słuchana piosenka",
    ),
    "musicAndPlayback": MessageLookupByLibrary.simpleMessage(
      "Muzyka i Odtwarzanie",
    ),
    "musicRecognition": MessageLookupByLibrary.simpleMessage(
      "Rozpoznawanie muzyki",
    ),
    "networkError": MessageLookupByLibrary.simpleMessage(
      "Błąd sieci! Sprawdź połączenie sieciowe.",
    ),
    "networkError1": MessageLookupByLibrary.simpleMessage("Ups, błąd sieci!"),
    "newVersionAvailable": MessageLookupByLibrary.simpleMessage(
      "Nowa wersja już dostępna!",
    ),
    "news_btn_app": MessageLookupByLibrary.simpleMessage(
      "Aplikacja Joss Red (Sklep Play)",
    ),
    "news_btn_dismiss": MessageLookupByLibrary.simpleMessage("Zrozumiany"),
    "news_btn_web": MessageLookupByLibrary.simpleMessage("Joss Red Web"),
    "news_card_subtitle": MessageLookupByLibrary.simpleMessage(
      "100% synchronizacja z Joss Red, playlisty ze znajomymi i wiele więcej. Kliknij, aby zobaczyć, co nowego.",
    ),
    "news_card_title": MessageLookupByLibrary.simpleMessage(
      "Mo Music ewoluowała!",
    ),
    "news_dialog_friends_desc": MessageLookupByLibrary.simpleMessage(
      "Aby dodawać znajomych, akceptować prośby lub zarządzać swoim profilem bezpieczeństwa, skorzystaj z Joss Red na jego oficjalnych platformach:",
    ),
    "news_dialog_section_friends": MessageLookupByLibrary.simpleMessage(
      "Przyjaciele i zarządzanie kontem:",
    ),
    "news_dialog_title": MessageLookupByLibrary.simpleMessage(
      "Wiadomości muzyczne Estrelli",
    ),
    "news_item_collab_desc": MessageLookupByLibrary.simpleMessage(
      "Twórz playlisty ze znajomymi! Tworząc playlistę, zaznacz pole wyboru Współpraca i wybierz znajomych, których chcesz wspólnie edytować.",
    ),
    "news_item_collab_title": MessageLookupByLibrary.simpleMessage(
      "Wspólne playlisty",
    ),
    "news_item_sync_desc": MessageLookupByLibrary.simpleMessage(
      "Twoje playlisty i ulubione są teraz automatycznie zapisywane i synchronizowane w chmurze z Twoim głównym kontem Joss Red.",
    ),
    "news_item_sync_title": MessageLookupByLibrary.simpleMessage(
      "Pełna integracja z Joss Red",
    ),
    "news_item_trans_desc": MessageLookupByLibrary.simpleMessage(
      "Nie musisz już klikać przycisków ręcznej synchronizacji; Nowy silnik odpowiada za automatyczną zmianę biegów w górę i w dół.",
    ),
    "news_item_trans_title": MessageLookupByLibrary.simpleMessage(
      "Przejrzysta synchronizacja",
    ),
    "no": MessageLookupByLibrary.simpleMessage("Nie"),
    "noBookmarks": MessageLookupByLibrary.simpleMessage("Brak zakładek!"),
    "noJossRedFriends": MessageLookupByLibrary.simpleMessage(
      "Nie masz dodanych znajomych na Joss Red.",
    ),
    "noLibPlaylist": MessageLookupByLibrary.simpleMessage(
      "Nie masz żadnej playlisty!",
    ),
    "noMatchInstruction": MessageLookupByLibrary.simpleMessage(
      "Nie udało się znaleźć żadnych utworów w nagranym dźwięku",
    ),
    "noMatchesFound": MessageLookupByLibrary.simpleMessage("Brak dopasowań"),
    "noOfflineSong": MessageLookupByLibrary.simpleMessage(
      "Brak utworów offline!",
    ),
    "noSongsInCollection": MessageLookupByLibrary.simpleMessage(
      "W tej kolekcji nie ma żadnych utworów",
    ),
    "nomatch": MessageLookupByLibrary.simpleMessage(
      "Nie znaleziono dopasowania dla",
    ),
    "notAuthenticated": MessageLookupByLibrary.simpleMessage(
      "Nieuwierzytelnione",
    ),
    "notaSongVideo": MessageLookupByLibrary.simpleMessage(
      "To nie utwór muzyczny/teledysk!",
    ),
    "notaValidLink": MessageLookupByLibrary.simpleMessage(
      "Nieprawidłowy link!",
    ),
    "openIn": MessageLookupByLibrary.simpleMessage("Otwórz w"),
    "operationFailed": MessageLookupByLibrary.simpleMessage(
      "Operacja zakończyła się niepowodzeniem",
    ),
    "password": MessageLookupByLibrary.simpleMessage("Hasło"),
    "password_text": MessageLookupByLibrary.simpleMessage("Hasło"),
    "permissionDenied": MessageLookupByLibrary.simpleMessage(
      "Odmowa pozwolenia",
    ),
    "permissionsAllow": MessageLookupByLibrary.simpleMessage("Umożliwić"),
    "permissionsConsentDescription": MessageLookupByLibrary.simpleMessage(
      "Mo Music potrzebuje tych uprawnień, aby zarządzać Twoją muzyką i oferować wszystkie funkcje odtwarzania.",
    ),
    "permissionsConsentTitle": MessageLookupByLibrary.simpleMessage(
      "Uprawnienia do rozpoczęcia",
    ),
    "permissionsContinueButton": MessageLookupByLibrary.simpleMessage(
      "Przyznaj wymagane uprawnienia",
    ),
    "permissionsMicrophoneDescription": MessageLookupByLibrary.simpleMessage(
      "Jest używany tylko wtedy, gdy zdecydujesz się zidentyfikować piosenkę odtwarzaną wokół ciebie.",
    ),
    "permissionsMicrophoneTitle": MessageLookupByLibrary.simpleMessage(
      "Mikrofon",
    ),
    "permissionsNotificationsDescription": MessageLookupByLibrary.simpleMessage(
      "Pokazuje elementy sterujące odtwarzaniem, postęp pobierania i ważne powiadomienia o aplikacjach.",
    ),
    "permissionsNotificationsTitle": MessageLookupByLibrary.simpleMessage(
      "Powiadomienia",
    ),
    "permissionsOpenSettings": MessageLookupByLibrary.simpleMessage(
      "Ustawienia",
    ),
    "permissionsRequiredNotice": MessageLookupByLibrary.simpleMessage(
      "Aby kontynuować, wymagane są wszystkie trzy zezwolenia. Możesz je później zmienić w ustawieniach systemu.",
    ),
    "permissionsStorageDescription": MessageLookupByLibrary.simpleMessage(
      "Umożliwia odtwarzanie muzyki, zapisywanie pobranych plików, eksportowanie list odtwarzania i przygotowywanie aktualizacji.",
    ),
    "permissionsStorageTitle": MessageLookupByLibrary.simpleMessage(
      "Muzyka i przechowywanie",
    ),
    "personalisation": MessageLookupByLibrary.simpleMessage("Personalizacja"),
    "pipedPlaylistDescription": MessageLookupByLibrary.simpleMessage(
      "Przesłana playlista",
    ),
    "pipedplstSyncAlert": MessageLookupByLibrary.simpleMessage(
      "Zsynchronizowano playlisty z Piped!",
    ),
    "plain": MessageLookupByLibrary.simpleMessage("Zwykły"),
    "play": MessageLookupByLibrary.simpleMessage("Zagraj"),
    "playNext": MessageLookupByLibrary.simpleMessage("Odtwórz następny"),
    "playNow": MessageLookupByLibrary.simpleMessage("Zagraj teraz"),
    "playbackSpeed": MessageLookupByLibrary.simpleMessage(
      "Szybkość odtwarzania",
    ),
    "playerUi": MessageLookupByLibrary.simpleMessage(
      "Interfejs użytkownika gracza",
    ),
    "playerUiDes": MessageLookupByLibrary.simpleMessage(
      "Wybierz interfejs użytkownika odtwarzacza",
    ),
    "playingRecognizedTrack": MessageLookupByLibrary.simpleMessage("Gra:"),
    "playingfromAlbum": MessageLookupByLibrary.simpleMessage(
      "ODTWARZANIE Z ALBUMU",
    ),
    "playingfromArtist": MessageLookupByLibrary.simpleMessage(
      "ODTWARZANIE WG. ARTYSTY",
    ),
    "playingfromPlaylist": MessageLookupByLibrary.simpleMessage(
      "ODTWARZANIE Z PLAYLISTY",
    ),
    "playingfromSelection": MessageLookupByLibrary.simpleMessage(
      "ODTWARZANIE Z WYBRANEGO ZESTAWU",
    ),
    "playlist": MessageLookupByLibrary.simpleMessage("Lista odtwarzania"),
    "playlistBlacklistAlert": MessageLookupByLibrary.simpleMessage(
      "Dodaj playlistę do czarnej listy!",
    ),
    "playlistBookmarkAddAlert": MessageLookupByLibrary.simpleMessage(
      "Dodano Playlistę do zakładek!",
    ),
    "playlistBookmarkRemoveAlert": MessageLookupByLibrary.simpleMessage(
      "Usunięto Playlistę z zakładek!",
    ),
    "playlistCollaboratorsTitle": MessageLookupByLibrary.simpleMessage(
      "Współtwórcy playlisty",
    ),
    "playlistCreatedAlert": MessageLookupByLibrary.simpleMessage(
      "Playlista została utworzona!",
    ),
    "playlistCreatednsongAddedAlert": MessageLookupByLibrary.simpleMessage(
      "Utworzono playlistę i dodano utwór!",
    ),
    "playlistExportedMsg": MessageLookupByLibrary.simpleMessage(
      "Lista odtwarzania została pomyślnie wyeksportowana do",
    ),
    "playlistImportedMsg": MessageLookupByLibrary.simpleMessage(
      "Lista odtwarzania została pomyślnie zaimportowana",
    ),
    "playlistRemovedAlert": MessageLookupByLibrary.simpleMessage(
      "Usunięto Playlistę!",
    ),
    "playlistRenameAlert": MessageLookupByLibrary.simpleMessage(
      "Zmiana nazwy zakończona powodzeniem!",
    ),
    "playlists": MessageLookupByLibrary.simpleMessage("Playlisty"),
    "playnextMsg": MessageLookupByLibrary.simpleMessage("Nadchodzące"),
    "podcasts": MessageLookupByLibrary.simpleMessage("Podcasty"),
    "popularTracks": MessageLookupByLibrary.simpleMessage("Popularne utwory"),
    "processFiles": MessageLookupByLibrary.simpleMessage(
      "Przetwarzanie plików...",
    ),
    "processingAudio": MessageLookupByLibrary.simpleMessage(
      "Przetwarzanie dźwięku...",
    ),
    "profiles": MessageLookupByLibrary.simpleMessage("Profile"),
    "queueLoop": MessageLookupByLibrary.simpleMessage("Pętla kolejki"),
    "queueLoopNotDisMsg1": MessageLookupByLibrary.simpleMessage(
      "Trybu pętli kolejki nie można wyłączyć, gdy włączony jest tryb losowy.",
    ),
    "queueLoopNotDisMsg2": MessageLookupByLibrary.simpleMessage(
      "Tryb pętli kolejki nie może być włączony w trybie radiowym.",
    ),
    "queueShufflingDeniedMsg": MessageLookupByLibrary.simpleMessage(
      "Nie można zmienić kolejności kolejki, gdy włączony jest tryb losowy",
    ),
    "queuerearrangingDeniedMsg": MessageLookupByLibrary.simpleMessage(
      "Nie można zmienić kolejności kolejki, gdy włączony jest tryb losowy",
    ),
    "quickPics": MessageLookupByLibrary.simpleMessage("Chwytliwe fotki"),
    "quickpicks": MessageLookupByLibrary.simpleMessage("Szybki wybór"),
    "radioNotAvailable": MessageLookupByLibrary.simpleMessage(
      "Radio niedostępne dla tego wykonawcy!",
    ),
    "randomRadio": MessageLookupByLibrary.simpleMessage("Losowe Radio"),
    "randomSelection": MessageLookupByLibrary.simpleMessage("Wybór losowy"),
    "reArrangePlaylist": MessageLookupByLibrary.simpleMessage(
      "Zmień kolejność listy odtwarzania",
    ),
    "reArrangeSongs": MessageLookupByLibrary.simpleMessage(
      "Zmień kolejność utworów",
    ),
    "readMore": MessageLookupByLibrary.simpleMessage("Przeczytaj więcej"),
    "recentSearches": MessageLookupByLibrary.simpleMessage(
      "Ostatnie wyszukiwania",
    ),
    "recentlyPlayed": MessageLookupByLibrary.simpleMessage(
      "Ostatnio odtwarzane",
    ),
    "recommend_cloud": MessageLookupByLibrary.simpleMessage(
      "Zalecamy aktywację trybu chmury, aby uzyskać wrażenia podobne do Spotify: synchronizację w czasie rzeczywistym między wszystkimi urządzeniami i automatyczne tworzenie kopii zapasowych bez konieczności wykonywania jakichkolwiek czynności.",
    ),
    "recommendedAlbums": MessageLookupByLibrary.simpleMessage("Zalecony"),
    "recommendedArtists": MessageLookupByLibrary.simpleMessage("Zalecony"),
    "removeFromCache": MessageLookupByLibrary.simpleMessage(
      "Usuń z pamięci podręcznej",
    ),
    "removeFromLib": MessageLookupByLibrary.simpleMessage(
      "Usuń z Biblioteki Utworów",
    ),
    "removeFromLibrary": MessageLookupByLibrary.simpleMessage(
      "Usuń z biblioteki",
    ),
    "removeFromPlaylist": MessageLookupByLibrary.simpleMessage(
      "Usuń z playlistę",
    ),
    "removeFromQueue": MessageLookupByLibrary.simpleMessage("Usuń z kolejki"),
    "removeMultiple": MessageLookupByLibrary.simpleMessage(
      "Usuń wiele utworów",
    ),
    "removePlaylist": MessageLookupByLibrary.simpleMessage("Usuń playlistę"),
    "rename": MessageLookupByLibrary.simpleMessage("Zmień nazwę"),
    "renamePlaylist": MessageLookupByLibrary.simpleMessage(
      "Zmień nazwę playlisty",
    ),
    "reproducedBy": MessageLookupByLibrary.simpleMessage("Reprodukowane przez"),
    "reset": MessageLookupByLibrary.simpleMessage("Resetuj"),
    "resetToDefault": MessageLookupByLibrary.simpleMessage(
      "Przywracanie ustawień domyślnych",
    ),
    "resetToDefaultDes": MessageLookupByLibrary.simpleMessage(
      "Przywróć domyślne ustawienia aplikacji (wymagane ponowne uruchomienie)",
    ),
    "resetToDefaultMsg": MessageLookupByLibrary.simpleMessage(
      "Przywracanie ustawień domyślnych zakończone, uruchom ponownie aplikację",
    ),
    "resetblacklistedplaylist": MessageLookupByLibrary.simpleMessage(
      "Usuń playlistę z czarnej listy",
    ),
    "resetblacklistedplaylistDes": MessageLookupByLibrary.simpleMessage(
      "Zresetuj wszystkie potokowane playlisty z czarnej listy",
    ),
    "restartApp": MessageLookupByLibrary.simpleMessage("Restart aplikacji"),
    "restore": MessageLookupByLibrary.simpleMessage("Przywracanie"),
    "restoreAppData": MessageLookupByLibrary.simpleMessage(
      "Przywracanie danych aplikacji",
    ),
    "restoreLastPlaybackSession": MessageLookupByLibrary.simpleMessage(
      "Przywróć ostatnią sesję odtwarzania",
    ),
    "restoreLastPlaybackSessionDes": MessageLookupByLibrary.simpleMessage(
      "Automatyczne przywracanie ostatniej sesji odtwarzania po uruchomieniu aplikacji",
    ),
    "restoreMsg": MessageLookupByLibrary.simpleMessage(
      "Przywrócenie powiodło się!\nZmiany zostaną zastosowane po ponownym uruchomieniu",
    ),
    "restoreSettingsAndPlaylists": MessageLookupByLibrary.simpleMessage(
      "Przywracanie ustawień i list odtwarzania",
    ),
    "restoreSettingsAndPlaylistsDes": MessageLookupByLibrary.simpleMessage(
      "Przywraca wszystkie ustawienia, dane logowania i listy odtwarzania z pliku kopii zapasowej. Nadpisuje wszystkie bieżące dane",
    ),
    "restore_select_file_dialog": MessageLookupByLibrary.simpleMessage(
      "Wybierz plik kopii zapasowej",
    ),
    "restoring": MessageLookupByLibrary.simpleMessage("Przywracanie..."),
    "results": MessageLookupByLibrary.simpleMessage("Wyniki"),
    "retry": MessageLookupByLibrary.simpleMessage("Spróbuj ponownie!"),
    "save": MessageLookupByLibrary.simpleMessage("Trzymać"),
    "savedAlbums": MessageLookupByLibrary.simpleMessage("Zapisano"),
    "scanning": MessageLookupByLibrary.simpleMessage("Skanowanie..."),
    "search": MessageLookupByLibrary.simpleMessage("Szukaj"),
    "searchDes": MessageLookupByLibrary.simpleMessage(
      "Utwory, Playlista, Album lub Wykonawca",
    ),
    "searchInLibrary": MessageLookupByLibrary.simpleMessage(
      "Szukaj w Bibliotece",
    ),
    "searchRes": MessageLookupByLibrary.simpleMessage("Wyniki wyszukiwania"),
    "search_recent_title": MessageLookupByLibrary.simpleMessage(
      "Ostatnie wyszukiwania",
    ),
    "selectAll": MessageLookupByLibrary.simpleMessage("Zaznacz wszystko"),
    "selectAuthIns": MessageLookupByLibrary.simpleMessage(
      "Wybierz instancję do uwierzytelniania",
    ),
    "selectAuthInsMsg": MessageLookupByLibrary.simpleMessage(
      "Należy wybrać instancję uwierzytelniania!",
    ),
    "selectFile": MessageLookupByLibrary.simpleMessage("Wybierz plik"),
    "selectSongs": MessageLookupByLibrary.simpleMessage("Wybierz utwory"),
    "selectedFileNotFound": MessageLookupByLibrary.simpleMessage(
      "Nie znaleziono wybranego pliku.",
    ),
    "sessionExpiredLoginAgain": MessageLookupByLibrary.simpleMessage(
      "Twoja sesja wygasła. Zaloguj się ponownie.",
    ),
    "setDiscoverContent": MessageLookupByLibrary.simpleMessage(
      "Wykryj zawartość",
    ),
    "settings": MessageLookupByLibrary.simpleMessage("Ustawienia"),
    "settings_about_desc": MessageLookupByLibrary.simpleMessage(
      "O muzyce Estrella",
    ),
    "settings_about_sub": MessageLookupByLibrary.simpleMessage(
      "Wersja, projekt open source i GitHub.",
    ),
    "settings_account_desc": MessageLookupByLibrary.simpleMessage(
      "Konto i synchronizacja",
    ),
    "settings_account_sub": MessageLookupByLibrary.simpleMessage(
      "Tryb chmury, kopie zapasowe, lista znajomych i migracje.",
    ),
    "settings_appearance_desc": MessageLookupByLibrary.simpleMessage(
      "Animacje tematyczne, językowe i interfejsowe.",
    ),
    "settings_cloud_backup": MessageLookupByLibrary.simpleMessage(
      "Kopia zapasowa w chmurze",
    ),
    "settings_cloud_backup_desc": MessageLookupByLibrary.simpleMessage(
      "Przesyłaj, przywracaj i zarządzaj...",
    ),
    "settings_cloud_backup_dialog_desc": MessageLookupByLibrary.simpleMessage(
      "Prześlij kopię zapasową aplikacji w formacie .hmb na serwer i w razie potrzeby przywróć dowolną z zapisanych kopii zapasowych.",
    ),
    "settings_content_desc": MessageLookupByLibrary.simpleMessage(
      "Odkryj filtry, integrację z Piped i pamięciami podręcznymi.",
    ),
    "settings_downloads_desc": MessageLookupByLibrary.simpleMessage(
      "Pobieranie i przechowywanie",
    ),
    "settings_downloads_sub": MessageLookupByLibrary.simpleMessage(
      "Formaty audio, foldery i automatyczne pobieranie.",
    ),
    "settings_general_section": MessageLookupByLibrary.simpleMessage("Ogólny"),
    "settings_local_cloud_desc": MessageLookupByLibrary.simpleMessage(
      "Wybierz, przeprowadź migrację lub przejrzyj status synchronizacji za pomocą Joss Red.",
    ),
    "settings_local_cloud_title": MessageLookupByLibrary.simpleMessage(
      "Tryb lokalny / chmura EMusic",
    ),
    "settings_logout": MessageLookupByLibrary.simpleMessage("Wyloguj się"),
    "settings_migration_desc": MessageLookupByLibrary.simpleMessage(
      "Importuj playlisty, utwory...",
    ),
    "settings_migration_title": MessageLookupByLibrary.simpleMessage(
      "Przeprowadź migrację z Joss Music Kotlin",
    ),
    "settings_my_friends": MessageLookupByLibrary.simpleMessage(
      "moi przyjaciele",
    ),
    "settings_my_friends_desc": MessageLookupByLibrary.simpleMessage(
      "Zarządzaj bezpośrednio swoimi przyjaciółmi Joss Red.",
    ),
    "settings_playback_desc": MessageLookupByLibrary.simpleMessage(
      "Jakość przesyłania strumieniowego, normalizacja, wyciszenie i bateria.",
    ),
    "settings_refresh_visitor_desc": MessageLookupByLibrary.simpleMessage(
      "Wygeneruj ponownie swój identyfikator Online Music, jeśli treści Discover nie zostaną wczytane.",
    ),
    "settings_refresh_visitor_title": MessageLookupByLibrary.simpleMessage(
      "Odśwież identyfikator (identyfikator gościa)",
    ),
    "settings_visitor_error": MessageLookupByLibrary.simpleMessage("Błąd"),
    "settings_visitor_error_desc": MessageLookupByLibrary.simpleMessage(
      "Nie można wygenerować nowego identyfikatora. Spróbuj ponownie później.",
    ),
    "settings_visitor_exception": m7,
    "settings_visitor_updated": MessageLookupByLibrary.simpleMessage(
      "Zaktualizowany identyfikator",
    ),
    "settings_visitor_updated_desc": MessageLookupByLibrary.simpleMessage(
      "Nowy identyfikator gościa został pomyślnie wygenerowany.",
    ),
    "shareAlbum": MessageLookupByLibrary.simpleMessage("Udostępnij album"),
    "sharePlaylist": MessageLookupByLibrary.simpleMessage(
      "Udostępnij playlistę",
    ),
    "shareSong": MessageLookupByLibrary.simpleMessage("Udostępnij ten utwór"),
    "shazamSearching": MessageLookupByLibrary.simpleMessage(
      "Przeszukiwanie bazy danych Shazam w poszukiwaniu dopasowań...",
    ),
    "shuffle": MessageLookupByLibrary.simpleMessage("Losowe"),
    "shuffleQueue": MessageLookupByLibrary.simpleMessage("Przetasuj kolejkę"),
    "similarToTitle": m8,
    "singles": MessageLookupByLibrary.simpleMessage("Single"),
    "skipSilence": MessageLookupByLibrary.simpleMessage("Pomiń ciszę"),
    "skipSilenceDes": MessageLookupByLibrary.simpleMessage(
      "Cisza będzie pomijana podczas odtwarzania muzyki",
    ),
    "sleepTimeSetAlert": MessageLookupByLibrary.simpleMessage(
      "Wyłącznik czasowy jest ustawiony",
    ),
    "sleepTimer": MessageLookupByLibrary.simpleMessage("Wyłącznik czasowy"),
    "slide_indicator": m9,
    "songAddedToPlaylistAlert": MessageLookupByLibrary.simpleMessage(
      "Utwór dodano do playlisty!",
    ),
    "songAlreadyExists": MessageLookupByLibrary.simpleMessage(
      "Ten utwór już istnieje!",
    ),
    "songAlreadyOfflineAlert": MessageLookupByLibrary.simpleMessage(
      "Utwór jest już offline w pamięci podręcznej",
    ),
    "songEnqueueAlert": MessageLookupByLibrary.simpleMessage(
      "Utwór dodano do kolejki!",
    ),
    "songFound": MessageLookupByLibrary.simpleMessage("Znaleziono piosenkę!"),
    "songInfo": MessageLookupByLibrary.simpleMessage("Informacje o utworze"),
    "songNotPlayable": MessageLookupByLibrary.simpleMessage(
      "Nie można odtworzyć utworu z powodu ograniczeń serwera!",
    ),
    "songPitch": MessageLookupByLibrary.simpleMessage("ton piosenki"),
    "songRemovedAlert": MessageLookupByLibrary.simpleMessage("Usunięto z"),
    "songRemovedfromQueue": MessageLookupByLibrary.simpleMessage(
      "Usunięto z kolejki!",
    ),
    "songRemovedfromQueueCurrSong": MessageLookupByLibrary.simpleMessage(
      "Nie można usunąć aktualnie odtwarzanego utworu",
    ),
    "songs": MessageLookupByLibrary.simpleMessage("Utwory"),
    "songsImportedFromJossMusic": MessageLookupByLibrary.simpleMessage(
      "Utwory zaimportowane z Joss Music Kotlin",
    ),
    "sortAscendNDescend": MessageLookupByLibrary.simpleMessage(
      "Sortuj rosnąco/malejąco",
    ),
    "sortByDate": MessageLookupByLibrary.simpleMessage("Sortuj według daty"),
    "sortByDuration": MessageLookupByLibrary.simpleMessage(
      "Sortuj według czasu trwania",
    ),
    "sortByName": MessageLookupByLibrary.simpleMessage("Sortuj według nazwy"),
    "speedAndPitch": MessageLookupByLibrary.simpleMessage(
      "Szybkość i wysokość",
    ),
    "standard": MessageLookupByLibrary.simpleMessage("Standard"),
    "startRadio": MessageLookupByLibrary.simpleMessage("Uruchom radio"),
    "startupScreen": MessageLookupByLibrary.simpleMessage(
      "Otwórz przy uruchomieniu",
    ),
    "startupScreenDescription": MessageLookupByLibrary.simpleMessage(
      "Wybierz sekcję, którą Mo Music otworzy jako pierwszą",
    ),
    "status": MessageLookupByLibrary.simpleMessage("Stan"),
    "stopMusicOnTaskClear": MessageLookupByLibrary.simpleMessage(
      "Zatrzymaj muzykę",
    ),
    "stopMusicOnTaskClearDes": MessageLookupByLibrary.simpleMessage(
      "Odtwarzanie muzyki zostanie zatrzymane, gdy aplikacja zostanie usunięta z menedżera zadań",
    ),
    "streamingQuality": MessageLookupByLibrary.simpleMessage(
      "Jakość streamingu",
    ),
    "streamingQualityDes": MessageLookupByLibrary.simpleMessage(
      "Jakość strumienia muzyki",
    ),
    "subscribers": MessageLookupByLibrary.simpleMessage("subskrybentów"),
    "swipe_prompt": MessageLookupByLibrary.simpleMessage(
      "Przesuń, aby poznać opcje ➔",
    ),
    "syncChangesConfirmed": m10,
    "syncChangesSynced": m11,
    "syncCloudDownloadingExisting": MessageLookupByLibrary.simpleMessage(
      "Aktywowano tryb chmury. Pobieranie istniejącej biblioteki.",
    ),
    "syncCloudMigrationComplete": MessageLookupByLibrary.simpleMessage(
      "Aktywowano tryb chmury. Przeniesiona biblioteka.",
    ),
    "syncCloudModeActive": MessageLookupByLibrary.simpleMessage(
      "Tryb chmury aktywny",
    ),
    "syncCloudPending": MessageLookupByLibrary.simpleMessage(
      "Tryb chmury aktywny. Oczekująca synchronizacja.",
    ),
    "syncDownloadFailed": MessageLookupByLibrary.simpleMessage(
      "Nie udało się pobrać synchronizacji.",
    ),
    "syncDownloading": MessageLookupByLibrary.simpleMessage(
      "Pobieram zmiany w EMusic...",
    ),
    "syncForceReplaceBackupSaved": m12,
    "syncForceReplaceConfirmAction": MessageLookupByLibrary.simpleMessage(
      "Reemplazar y subir",
    ),
    "syncForceReplaceConfirmBody": MessageLookupByLibrary.simpleMessage(
      "Primero se creará un respaldo de recuperación. Después, las playlists, favoritos, historial, álbumes, artistas y ajustes musicales de EMusic Cloud se reemplazarán con los datos actuales de este dispositivo. No se puede deshacer desde el servidor.",
    ),
    "syncForceReplaceConfirmTitle": MessageLookupByLibrary.simpleMessage(
      "¿Reemplazar la biblioteca remota muzyczna?",
    ),
    "syncForceReplaceCountMismatch": MessageLookupByLibrary.simpleMessage(
      "Wgrane liczby nie pasują do lokalnej biblioteki. Zdalna wymiana nie mogła być potwierdzona.",
    ),
    "syncForceReplaceCreatingBackup": MessageLookupByLibrary.simpleMessage(
      "Tworzenie kopii zapasowej przed zastąpieniem danych w chmurze...",
    ),
    "syncForceReplaceDescription": MessageLookupByLibrary.simpleMessage(
      "Pausa la sincronización pendiente y reemplaza a la fuerza tu biblioteca musical remota con los datos actuales de este dispositivo. Las descargas permanecen locales.",
    ),
    "syncForceReplaceFailed": MessageLookupByLibrary.simpleMessage(
      "Emusic Cloud nie mógł zastąpić zdalnej biblioteki.",
    ),
    "syncForceReplaceFailedLocalPreserved": MessageLookupByLibrary.simpleMessage(
      "Zdalna wymiana nie powiodła się. Twoje lokalne dane i kopie zapasowe zostały zachowane.",
    ),
    "syncForceReplaceFailedTitle": MessageLookupByLibrary.simpleMessage(
      "Wysyłanie nie zakończone",
    ),
    "syncForceReplaceInProgress": MessageLookupByLibrary.simpleMessage(
      "Przerwanie synchronizacji, tworzenie kopii zapasowych i wysyłanie lokalnej biblioteki...",
    ),
    "syncForceReplacePauseFailed": MessageLookupByLibrary.simpleMessage(
      "Obecna synchronizacja nie mogła być bezpiecznie wstrzymana. Spróbuj jeszcze raz za chwilę.",
    ),
    "syncForceReplaceSuccess": MessageLookupByLibrary.simpleMessage(
      "Zdalna biblioteka muzyczna została zastąpiona bieżącymi danymi tego urządzenia.",
    ),
    "syncForceReplaceSuccessTitle": MessageLookupByLibrary.simpleMessage(
      "Wysyłanie zakończone",
    ),
    "syncForceReplaceTitle": MessageLookupByLibrary.simpleMessage(
      "Cancelar sincronización y subir esta base",
    ),
    "syncForceReplaceValidating": MessageLookupByLibrary.simpleMessage(
      "Walidacja przesłanej biblioteki przed zastąpieniem danych w chmurze...",
    ),
    "syncLibrarySynced": MessageLookupByLibrary.simpleMessage(
      "Zsynchronizowana biblioteka.",
    ),
    "syncLibraryUpToDate": MessageLookupByLibrary.simpleMessage(
      "Biblioteka na bieżąco.",
    ),
    "syncLocalChangesFirst": MessageLookupByLibrary.simpleMessage(
      "Są nowe zmiany lokalne. Zostaną przesłane przed pobraniem.",
    ),
    "syncLocalDeviceOnly": MessageLookupByLibrary.simpleMessage(
      "Twoje dane są przechowywane wyłącznie na tym urządzeniu.",
    ),
    "syncLocalModeActive": MessageLookupByLibrary.simpleMessage(
      "Tryb lokalny aktywny",
    ),
    "syncOfflinePending": MessageLookupByLibrary.simpleMessage(
      "Nieaktywny. Zmiany oczekują.",
    ),
    "syncOfflineRetry": MessageLookupByLibrary.simpleMessage(
      "Nieaktywny. Zmiany zostały zapisane do ponownej próby.",
    ),
    "syncPlaylistSongs": MessageLookupByLibrary.simpleMessage(
      "Synchronizuj utwory z listy odtwarzania",
    ),
    "syncUnconfirmedRetry": MessageLookupByLibrary.simpleMessage(
      "EMusic nie potwierdził wszystkich zmian. Zostaną one rozpatrzone ponownie.",
    ),
    "syncUploadRetry": MessageLookupByLibrary.simpleMessage(
      "Nie mogłem wstać. Próba zostanie ponowiona później.",
    ),
    "syncUploadSuccess": MessageLookupByLibrary.simpleMessage(
      "Zmiany przesłane poprawnie.",
    ),
    "syncUploadSuccessWs": MessageLookupByLibrary.simpleMessage(
      "Zmiany przesłane pomyślnie (WS).",
    ),
    "syncUploadWsRetry": MessageLookupByLibrary.simpleMessage(
      "Nie można przesłać za pomocą WS. Próba zostanie ponowiona później.",
    ),
    "syncUploading": MessageLookupByLibrary.simpleMessage(
      "Przesyłanie zmian do EMusic...",
    ),
    "synced": MessageLookupByLibrary.simpleMessage("Zsynchronizowano"),
    "syncedLyricsNotAvailable": MessageLookupByLibrary.simpleMessage(
      "Synchronizowanie tekstu nie jest dostępne!",
    ),
    "systemDefault": MessageLookupByLibrary.simpleMessage(
      "Domyślne ustawienia systemowe",
    ),
    "themeMode": MessageLookupByLibrary.simpleMessage("Motyw"),
    "title": MessageLookupByLibrary.simpleMessage("Tytuł"),
    "topMusicVid": MessageLookupByLibrary.simpleMessage("Najlepsze teledyski"),
    "topmusicvideos": MessageLookupByLibrary.simpleMessage(
      "Najlepsze wideoklipy",
    ),
    "trending": MessageLookupByLibrary.simpleMessage("Trendy"),
    "unLink": MessageLookupByLibrary.simpleMessage("Odłącz"),
    "unlinkAlert": MessageLookupByLibrary.simpleMessage("Odłączono pomyślnie!"),
    "untitledSong": MessageLookupByLibrary.simpleMessage("Piosenka bez tytułu"),
    "upNext": MessageLookupByLibrary.simpleMessage("Następne"),
    "updateApp": MessageLookupByLibrary.simpleMessage("Zaktualizuj aplikację"),
    "urlSearchDes": MessageLookupByLibrary.simpleMessage(
      "Wykryty adres URL - kliknij go, aby otworzyć/odtworzyć powiązaną zawartość",
    ),
    "useThisMetadata": MessageLookupByLibrary.simpleMessage(
      "Użyj tych metadanych",
    ),
    "userBlocked": MessageLookupByLibrary.simpleMessage(
      "Zablokowany użytkownik",
    ),
    "userListMissing": MessageLookupByLibrary.simpleMessage(
      "Odpowiedź nie zawiera listy użytkowników.",
    ),
    "userSearchFailed": m13,
    "userUnblocked": MessageLookupByLibrary.simpleMessage(
      "Odblokowany użytkownik",
    ),
    "username": MessageLookupByLibrary.simpleMessage("Nazwa użytkownika"),
    "video": MessageLookupByLibrary.simpleMessage("Wideo"),
    "videos": MessageLookupByLibrary.simpleMessage("Teledyski"),
    "viewAll": MessageLookupByLibrary.simpleMessage("Pokaż wszystko"),
    "viewArtist": MessageLookupByLibrary.simpleMessage("Zobacz wykonawcę"),
    "welcome_intro": MessageLookupByLibrary.simpleMessage(
      "Zmodernizowaliśmy naszą platformę. Stary system ręcznego przesyłania kopii zapasowych został wyłączony. Masz teraz dwa jasne sposoby zarządzania swoją biblioteką muzyczną.",
    ),
    "welcome_subtitle": MessageLookupByLibrary.simpleMessage(
      "Wybierz, w jaki sposób chcesz odtąd korzystać z Mo Music.",
    ),
    "welcome_title": MessageLookupByLibrary.simpleMessage(
      "Twoja muzyka, na Twój sposób",
    ),
  };
}
