...
class ArtistContentListScreen extends StatelessWidget {
  ...
  Widget _getContentTile(
      BuildContext context, dynamic item, int index, List<dynamic> allItems) {
    final playerController = Get.find<PlayerController>();
    final videoId =
        (item is MediaItem) ? item.id : (item?['videoId'] as String? ?? '');
    ...
    return Obx(() {
      ...
      if (item is MediaItem || (item is Map && item.containsKey('videoId'))) {
        final title =
            (item is MediaItem) ? item.title : (item['title'] as String? ?? '');
        final artist = (item is MediaItem)
            ? item.artist
            : (item['artists']?.map((e) => e['name']).join(', ') ?? '');
        final thumbUrl = (item is MediaItem)
        ...
        return InkWell(
          onTap: () {
            final mediaItems = allItems
                .where((e) =>
                    e is MediaItem || (e is Map && e.containsKey('videoId')))
                .map((e) {
          ...
          child: AnimatedContainer(
            ...
            child: Row(
              children: [
                ...
                ValueListenableBuilder(
                  valueListenable:
                      SqliteStore.box("SongDownloads").listenable(),
                  builder: (context, SqliteBox box, _) {
        ...
        final title =
            (item is Playlist) ? item.title : (item['title'] as String? ?? '');
